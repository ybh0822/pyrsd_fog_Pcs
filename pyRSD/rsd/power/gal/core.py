...
class DampedGalaxyPowerTerm_fixed(GalaxyPowerTerm):
    """
    A class to represent a damped, galaxy power spectrum term,
    which is a `GalaxyPowerTerm`, with FOG kernels applied
    """
    ...
    def __init__(self, model, *terms, **kws):
        ...
        *terms :
        ...
        super(DampedGalaxyPowerTerm_fixed, self).__init__(model, *terms)
        self._sigma1_name = sigma1
        self._sigma2_name = sigma2
    ...
    def __call__(self, k, mu):
        ...
        Return the damped power spectrum
        """
        G1 = G2 = self.model.FOG_fixed(k, mu, self.sigma1)
        if self.sigma2 is not None:
            G2 = self.model.FOG_fixed(k, mu, self.sigma2)

        toret = super(DampedGalaxyPowerTerm_fixed, self).__call__(k, mu)
        return G1*G2 * toret + self.model.N
    ...
    def derivative_k(self, k, mu):
        ...
        # FOG and derivative
        G1      = G2 = self.model.FOG_fixed(k, mu, self.sigma1)
        G1prime = G2prime = self.model.FOG_fixed.derivative_k(k, mu, self.sigma1)

        if self.sigma2 is not None:
            G2      = self.model.FOG_fixed(k, mu, self.sigma2)
            G2prime = self.model.FOG_fixed.derivative_k(k, mu, self.sigma2)

        deriv = super(DampedGalaxyPowerTerm_fixed, self).derivative_k(k, mu)
        power = super(DampedGalaxyPowerTerm_fixed, self).__call__(k, mu)
    ...
    def derivative_mu(self, k, mu):
        ...
        """
        G1      = G2 = self.model.FOG_fixed(k, mu, self.sigma1)
        G1prime = G2prime = self.model.FOG_fixed.derivative_mu(k, mu, self.sigma1)

        if self.sigma2 is not None:
            G2      = self.model.FOG_fixed(k, mu, self.sigma2)
            G2prime = self.model.FOG_fixed.derivative_mu(k, mu, self.sigma2)

        deriv = super(DampedGalaxyPowerTerm_fixed, self).derivative_mu(k, mu)
        power = super(DampedGalaxyPowerTerm_fixed, self).__call__(k, mu)
